...
func lockIntersectionInDistance(id, reserveStart, reserveEnd int, crossings []*common.Crossing) {
	...
	//this is where the train locks the intersections it needs to cross
	for _, it := range intersectionsToLock {
		it.LockedBy = id
		time.Sleep(10 * time.Millisecond) //preventing deadlock by sleeping for 10 milliseconds after locking each intersection
	}
	controller.Unlock()
}
